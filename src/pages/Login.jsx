        ...
        <form onSubmit={handleSubmit} className="w-full max-w-sm space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-seka-text-muted"/>
              <input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="votre@email.com"
                className="input-seka pl-12"
                required
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-seka-text-muted"/>
              <input
                type={showPassword ? 'text' : 'password'}
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                placeholder="••••••••"
                className="input-seka pl-12 pr-12"
                required
                minLength={6}
              />
              <button
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 text-seka-text-muted"
              >
                {showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
              </button>
            </div>
            {!isLogin && (
              <p className="text-xs text-seka-text-muted mt-1">Minimum 6 caractères</p>
            )}
          </div>
          ...
          <button
            ...
            className="w-full py-4 rounded-xl bg-seka-gradient text-seka-darker font-semibold flex items-center justify-center gap-2 disabled:opacity-50"
          >
