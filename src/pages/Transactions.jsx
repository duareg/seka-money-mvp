...
import api, { formatMoney, formatDate, EXPENSE_CATEGORIES, INCOME_CATEGORIES } from '../utils/api'
...
export default function Transactions() {
  ...
  const loadData = async () => {
    try {
      ...
      const apiTrans = res.data.items || []
      setTransactions(apiTrans) // plus de fallback DEMO
    } catch (e) {
      console.error('Erreur chargement transactions:', e)
      setTransactions([]) // en cas d’erreur, liste vide
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className={`min-h-screen pb-24 ${isDark ? 'bg-seka-dark' : 'bg-gray-50'}`}>
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              {PERIODS.map(p => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs whitespace-nowrap transition-all ${period === p.key
          ...
          <div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 flex-wrap">
              <button
                ...
                className={`px-3 py-2 rounded-lg text-xs whitespace-nowrap transition-all flex items-center gap-1 ${categoryFilter === 'all'
                    ? 'bg-seka-green text-seka-darker font-medium'
                    : isDark
                      ? 'bg-seka-darker text-seka-text-secondary'
                      : 'bg-gray-100 text-gray-600'
              ...
              {allCategories
                ...
                .map(c => (
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg text-xs whitespace-nowrap transition-all flex items-center gap-1 ${categoryFilter === c.key
                        ? 'bg-seka-gold/20 text-seka-gold border border-seka-gold font-medium'
                        : isDark
                          ? 'bg-seka-darker text-seka-text-secondary'
                          : 'bg-gray-100 text-gray-600'
                      }`}
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              {PAYMENT_METHODS.map(p => (
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-xs whitespace-nowrap transition-all flex items-center gap-1 ${paymentFilter === p.key
                      ? 'bg-purple-500/20 text-purple-400 border border-purple-500 font-medium'
                      : isDark
                        ? 'bg-seka-darker text-seka-text-secondary'
                        : 'bg-gray-100 text-gray-600'
                    }`}
      ...
      <div className="px-4 py-3 grid grid-cols-2 gap-3">
        <div className={`p-3 rounded-xl ${isDark ? 'bg-seka-card/50 border border-seka-border' : 'bg-white shadow-sm border border-gray-100'}`}>
          ...
          <p className="text-sm font-bold text-seka-green font-mono">
        ...
        <div className={`p-3 rounded-xl ${isDark ? 'bg-seka-card/50 border border-seka-border' : 'bg-white shadow-sm border border-gray-100'}`}>
          ...
          <p className="text-sm font-bold text-seka-red font-mono">
        ...
        <div className="px-4 pb-2">
          <div className="flex items-center gap-2 py-2 px-3 bg-seka-gold/10 rounded-lg border border-seka-gold/30">
            <span className="text-lg">
              {(EXPENSE_CATEGORIES[categoryFilter] || INCOME_CATEGORIES[categoryFilter])?.emoji}
            </span>
            <span className="text-xs text-seka-gold flex-1">
              {(EXPENSE_CATEGORIES[categoryFilter] || INCOME_CATEGORIES[categoryFilter])?.label}
            </span>
            <button onClick={() => setCategoryFilter('all')}>
              <X className="w-4 h-4 text-seka-gold" />
            </button>
          </div>
        </div>
      )}
      ...
      <div className="px-4 space-y-2">
        {filteredTransactions.length === 0 ? (
          <div className={`p-8 text-center rounded-xl ${isDark ? 'bg-seka-card/50 border border-seka-border' : 'bg-white shadow-sm border border-gray-100'}`}>
            ...
            <p className={`text-xs mt-1 ${isDark ? 'text-seka-text-muted' : 'text-gray-400'}`}>
          ...
          filteredTransactions.map(t => {
            ...
            return (
              <div
                key={t.id}
                className={`p-3 flex items-center gap-3 rounded-xl ${isDark ? 'bg-seka-card/50 border border-seka-border' : 'bg-white shadow-sm border border-gray-100'
                  }`}
              >
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg ${isDark ? 'bg-seka-darker' : 'bg-gray-100'}`}>
                  {cat.emoji}
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`font-medium text-sm truncate ${isDark ? 'text-seka-text' : 'text-gray-900'}`}>
                    {cat.label}
                  </p>
                ...
                <div className="text-right">
                  <p className={`font-mono font-semibold text-sm ${isExp ? 'text-seka-red' : 'text-seka-green'}`}>
