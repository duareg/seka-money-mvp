...
export default function Dashboard() {
  ...
  const [expensePeriod, setExpensePeriod] = useState(6) // 3, 6, 12 mois
  ...
  const loadData = async () => {
    try {
      ...
      // IMPORTANT : plus de DEMO_TRANSACTIONS, seulement tes vraies données
  ...
  const epargne = stats.income - stats.expense
  const financialScore = stats.score
  ...
  return (
    <div className={`px-4 pt-5 pb-4 space-y-4 ${isDark ? '' : 'bg-gray-50'}`}>
          ...
          <div className="flex justify-between items-start">
            ...
            >
              ...
              <div className="flex flex-col items-end leading-none">
                ...
                <span className="text-[8px] text-seka-gold">5000F/an</span>
              </div>
            </Link>
          </div>
        ...
        <div className="bg-gradient-to-r from-indigo-900/40 to-purple-900/40 border border-indigo-500/20 p-5 rounded-2xl relative overflow-hidden active:scale-[0.98] transition-transform duration-200">
          ...
          <div className="flex justify-between items-center relative z-10 mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-seka-text text-sm">Score Financier</h3>
            ...
            <div className="text-right">
              <span className="text-2xl font-bold text-seka-gold">
                {financialScore}
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        >
          <PiggyBank
            className={`w-4 h-4 mb-1 ${epargne >= 0 ? 'text-seka-green' : 'text-seka-red'}`}
          />
          <p className={`text-[10px] ${isDark ? 'text-seka-text-muted' : 'text-gray-500'}`}>
            Épargne
          </p>
          <p
            className={`text-sm font-bold font-mono ${epargne >= 0 ? 'text-seka-green' : 'text-seka-red'
              }`}
          >
            {epargne >= 0 ? '+' : ''}
            {formatMoney(epargne)}
            ...
            {stats.topCategories.map((cat, i) => (
              <div key={i}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span
                    className={`text-xs font-mono ${isDark ? 'text-seka-text-muted' : 'text-gray-500'
                      }`}
                  >
                    {formatMoney(cat.amount)}
                  </span>
                  <span
                    className={`text-xs font-mono w-10 text-right ${isDark ? 'text-seka-text' : 'text-gray-700'
                      }`}
                  >
            ...
            recentTransactions.slice(0, 5).map(t => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`font-mono font-semibold text-xs ${isExp ? 'text-seka-red' : 'text-seka-green'
                      }`}
                  >
                    {isExp ? '-' : '+'}
